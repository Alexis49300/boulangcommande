...
function TabVerifFactures({ produits }) {
  ...
  const handleFile = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (!fournisseur) { alert("Veuillez sélectionner un fournisseur."); e.target.value=""; return; }
    setFilename(file.name);
    analyserFacture(file);
    e.target.value = "";
  };
  ...
  const analyserFacture = async (file) => {
    ...
    const msgs = [
      ...
      "Comparaison avec la mercuriale…",
      "Calcul des écarts…"
    ];
    let mi = 0;
  ...
  const ecartTotal = totalFacture - totalAttendu;
  const nbEcart = lignes.filter(l => l.statut === "surcharge" || l.statut === "remise").length;

  const fmt = (n) => Number(n).toFixed(2).replace(".", ",") + " €";

  const badgeStatut = (s) => {
    if (s === "ok") return { bg:"#EAF3DE", color:"#27500A", label:"✓ Conforme" };
    if (s === "surcharge") return { bg:"#FCEBEB", color:"#791F1F", label:"↑ Surcharge" };
    if (s === "remise") return { bg:"#FFF8E1", color:"#633806", label:"↓ Remise" };
    return { bg:"#f0f0f0", color:"#888", label:"— Non trouvé" };
  };

  const filterBtns = [
    { id:"tous", label:"Tous", count: lignes.length },
    { id:"ecart", label:"Écarts", count: lignes.filter(l=>l.statut==="surcharge"||l.statut==="remise").length },
    { id:"ok", label:"Conformes", count: lignes.filter(l=>l.statut==="ok").length },
    { id:"inconnu", label:"Non trouvés", count: lignes.filter(l=>l.statut==="inconnu").length },
  ];
    ...
    <div style={{ padding:16 }}>
        ...
        <>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16, flexWrap:"wrap" }}>
            <select value={fournisseur} onChange={e=>setFournisseur(e.target.value)}
              style={{ padding:"7px 12px", border:"2px solid #D4A96A", borderRadius:8, background:"#fffaf5", fontSize:13, color:"#2C1810" }}>
              <option value="">— Choisir le fournisseur —</option>
              {FOURNISSEURS_FACTURES.map(f => <option key={f} value={f}>{f}</option>)}
            </select>
            <div style={{ fontSize:11, color:"#9B7B5A" }}>Sélectionnez le fournisseur puis importez la facture PDF</div>
          </div>

          <div
            onClick={() => fileInputRef.current?.click()}
            style={{
              border:"2px dashed #D4A96A", borderRadius:12, padding:"3rem",
              textAlign:"center", cursor:"pointer", background:"#fffaf5",
              transition:"all .15s"
            }}
            onMouseEnter={e => { e.currentTarget.style.borderColor="#8B4513"; e.currentTarget.style.background="#FAF0E4"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor="#D4A96A"; e.currentTarget.style.background="#fffaf5"; }}
          >
            <div style={{ fontSize:36, marginBottom:8 }}>📄</div>
            <div style={{ fontSize:14, fontWeight:700, color:"#2C1810", marginBottom:4 }}>Déposer la facture PDF ici</div>
            <div style={{ fontSize:12, color:"#9B7B5A" }}>ou cliquer pour parcourir</div>
            <input ref={fileInputRef} type="file" accept=".pdf" onChange={handleFile} style={{ display:"none" }}/>
          </div>

          <div style={{ marginTop:16, padding:"12px 16px", background:"#E6F1FB", borderRadius:10, fontSize:12, color:"#0C447C", lineHeight:1.6 }}>
            <strong>Comment ça fonctionne</strong> — L'IA lit votre PDF de facture, extrait chaque ligne (référence, désignation, prix unitaire, quantité), puis compare avec les prix de votre mercuriale. Les surcharges apparaissent en rouge, les remises en orange.
          </div>
        </>
      )}
      ...
      {step === "loading" && (
        <div style={{ textAlign:"center", padding:"4rem 0" }}>
          ...
          <div style={{ fontSize:14, color:"#9B7B5A" }}>{loadingMsg}</div>
        </div>
      )}
      ...
      {step === "result" && (
        <>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16, flexWrap:"wrap", gap:8 }}>
            <div>
              <div style={{ fontSize:15, fontWeight:700, color:"#2C1810", fontFamily:"Georgia" }}>{fournisseur} — {filename}</div>
              <div style={{ fontSize:11, color:"#9B7B5A" }}>Analysée le {new Date().toLocaleDateString("fr-FR")}</div>
            </div>
            <button onClick={() => { setStep("upload"); setLignes([]); }}
              style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #D4A96A", background:"#fff", color:"#8B4513", cursor:"pointer", fontSize:12 }}>
              ← Nouvelle facture
            </button>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:10, marginBottom:16 }}>
            {[
              { label:"Total facturé", value:fmt(totalFacture), sub:`Attendu : ${fmt(totalAttendu)}`, color:"#2C1810" },
              { label:"Écart total", value:(ecartTotal>=0?"+":"")+fmt(ecartTotal), sub:ecartTotal>0.01?"Trop facturé":ecartTotal<-0.01?"Sous-facturé":"Conforme", color:ecartTotal>0.01?"#791F1F":ecartTotal<-0.01?"#27500A":"#2C1810" },
              { label:"Articles en écart", value:nbEcart, sub:`sur ${lignes.length} articles`, color:nbEcart>0?"#791F1F":"#27500A" },
            ].map((m,i) => (
              <div key={i} style={{ background:"#FAF6F0", borderRadius:10, padding:"12px 14px" }}>
                <div style={{ fontSize:11, color:"#9B7B5A", marginBottom:4 }}>{m.label}</div>
                <div style={{ fontSize:20, fontWeight:700, color:m.color }}>{m.value}</div>
                <div style={{ fontSize:11, color:"#9B7B5A", marginTop:2 }}>{m.sub}</div>
              </div>
            ))}
          </div>
            ...
            {filterBtns.map(b => (
              <button key={b.id} onClick={() => setFilter(b.id)}
                style={{
                  ...
                  border: filter===b.id ? "none" : "1px solid #D4A96A",
                  background: filter===b.id ? "#8B4513" : "#fff",
                  color: filter===b.id ? "#fff" : "#8B4513"
                }}>
                {b.label} ({b.count})
              </button>
            ))}
          ...
          {/* Tableau */}
          <div style={{ border:"1px solid #EDD5B3", borderRadius:12, overflow:"hidden" }}>
                ...
                {lignesFiltrees.map((l, i) => {
                  ...
                  return (
                    <tr key={l.origIndex} style={{ background:rowBg, borderTop:"1px solid #EDD5B3" }}>
                      ...
                      <td style={{ padding:"7px 12px", textAlign:"right", fontWeight:700, color:l.ecartTotal>0.01?"#791F1F":l.ecartTotal<-0.01?"#27500A":"#9B7B5A" }}>
                        {l.ecartTotal !== null ? (l.ecartTotal>=0?"+":"") + fmt(l.ecartTotal) : "—"}
                      </td>
                      <td style={{ padding:"7px 12px", textAlign:"center" }}>
                        <span style={{ padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:700, background:badge.bg, color:badge.color }}>
                          {badge.label}
                        </span>
                      </td>
                    </tr>
